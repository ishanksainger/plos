...
-- Hardening: this unique constraint makes fulfillment idempotent even under a
-- *simultaneous* /verify + webhook race — the second insert fails instead of
-- minting a duplicate token + email. lib/fulfillment.ts already guards every
-- realistic *sequential* duplicate with a select-then-insert check; this closes
-- the millisecond overlap window too.
-- Note: it collapses qty>1 of the same digital SKU on one order to a single
-- delivery (intended — a digital file is identical per copy). The bundle path
-- tolerates the conflict (component inserts `continue` on error; a standalone +
-- bundled copy of the same SKU is caught by the sequential guard first).
alter table commerce.order_items
  drop constraint if exists order_items_order_product_uniq;
alter table commerce.order_items
