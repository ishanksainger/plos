...
export class WhatsappService {
  ...
  async dispatchToUser(
    ...
    const user = await this.prisma.user.findUnique({
      ...
      select: {
        phone: true,
        notificationPrefs: { select: { whatsappOptIn: true } },
      },
    });

    if (!user?.phone) return { sent: false, reason: 'no_phone' };
    if (!user.notificationPrefs?.whatsappOptIn)
    ...
    if (!this.configured) {
      this.logger.log(
        `[whatsapp dormant — no provider] would send to ${user.phone}: ${msg.preview}`,
      );
    ...
    try {
      ...
      this.logger.error(
        `whatsapp send failed: ${err instanceof Error ? err.message : 'unknown'}`,
      );
