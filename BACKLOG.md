...
**Update 2026-06-03 — 2 of 3 CI jobs fixed → GREEN** (on `feat/plos-billing-readiness`, commits `ebad1e3` + `b63b136`; verified locally against the exact CI commands incl. a `postgres:16` container):
...
- [ ] **`plos-frontend` CI job is still RED — deeper, pre-existing, structural (NOT the billing work).** Needs a focused frontend pass (owner: `cursor/either`, P1 — blocks a green check on *every* frontend PR). Three layered problems, all reproduced locally 2026-06-03:
  1. **`npm ci` can't install** — `plos-frontend/package.json` declares `"@nis/ui": "*"`, but `@nis/ui` is a local workspace package (not published), and `plos-frontend` is **not** part of the root npm workspace. So `npm ci` 404s on `@nis/ui` from the registry — the job dies at install, before lint. (Locally this "works" only because Node walks up to the monorepo-root `node_modules` where the workspace symlinks `@nis/ui`; CI installs `plos-frontend` in isolation so that symlink never exists.) **Fix candidate:** `"@nis/ui": "file:../packages/ui"` (verified: makes `npm ci` resolve it). Note `@nis/ui` exports TS source (`./src/index.ts`) and imports `react`, so its react must resolve against the app's own copy in the isolated job — `tsconfig.app.json preserveSymlinks: true` worked for that but interacts with the type errors below; needs care.
  2. **Undeclared deps** — `src/components/three/*` import `@react-three/fiber` + `@react-three/drei` but neither is in `package.json` (phantom deps that only resolved via a hand-assembled local `node_modules`). A clean `npm ci` has neither → build fails. **Fix:** declare them (react-19 line: fiber `^9`, drei `^10`).
  3. **`tsc -b` type errors under the committed lockfile** — `npm run build` is `tsc -b && vite build`; `tsc -b` reports `style`-prop `never` errors on plain Mantine `<Box>` (PageHeader) and Tabler `<Icon>` (Sidebar) — a React-types resolution artifact present in a clean install of the committed lockfile (the dev's local `node_modules` predates it). Also note the separate `npx tsc --noEmit` CI step is a **no-op** (root tsconfig has `files: []`), so `tsc -b` is the only real typecheck. Needs diagnosis (likely an `@types/react`/Mantine version pin or targeted fixes).
- [ ] **`plos-frontend` lint debt** (separable from the build above) — `eslint-plugin-react-hooks` v7 turned on the new "React Compiler" rule family (purity/immutability/refs/set-state-in-effect) at error level; the existing charts/three.js code trips them (largely false positives for imperative three.js). **Fix candidate (verified green locally):** keep `rules-of-hooks` as error, demote the advisory rules + react-refresh + no-explicit-any/no-unused-vars to `warn` in `eslint.config.js`. **Owner:** `cursor/either` (P2). Bundle with the frontend build fix above.
