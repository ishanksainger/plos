...
6k. [code shipped · 2026-07-23 · claude · PR #39 — awaiting Ishank's price to go live] **Wedding Box: add the invitation-suite preview gallery** — Claude Desktop finished the four invitation designs (save-the-date · wedding invite · RSVP · thank-you, 1080×1350) for the **Wedding Box**, the premium bundle built around the live `wedding-budget` tracker (plus the 2 PDF guides + Start-Here page it already has). Wiring them up as a product gallery: web-optimised JPEGs into `public/trackers/wedding-box/`, a `wedding-box` entry in both catalogs staged **`active:false`** (invisible — grid/home/sitemap/detail all gate on `getPurchasableTracker`), and a "What's inside — the invitation suite" section on the detail page reusing the Lookbook `Carousel` (4:5 matches the art exactly). **Price is a flagged placeholder** pending Ishank; no Supabase `commerce.products` row and no deploy until go-live. **Corrections to the handoff brief** (recorded so the next one is right): (a) there is **no "bundles category"** in this codebase — it's a `kind: 'bundle'` field on the SKU; (b) product preview images do **not** go in Supabase Storage — that bucket is **private and token-gated** for paid deliverables (the Welcome PDFs); public art is committed to `apps/web/public/`; (c) there was no multi-image gallery on tracker pages before this — the field is new. **Owner:** `claude` (wiring) + Claude Desktop (the art, done).
...
- ~~**Self-owned visitor counter (no third-party account)**~~ → **shipped + deployed 2026-07-16 in PR #37 (`0895f08`).** First-party, privacy-clean pageview counter — `components/PageViewBeacon.tsx` (real-browser-only `sendBeacon`, no cookies, ephemeral `sessionStorage` id) → rate-limited fire-and-forget `POST /api/pv` → `marketing.page_views` in nis-prod (RLS on, no policies → only the service role reads/writes; anon blocked). A box cron (`/docker/analytics/pv-digest.py`, mirrors the keepalive: secrets sourced at runtime from `/docker/nis-web/.env` + `/docker/openclaw-skqf/.env`, never committed; daily 09:05 IST = `35 3 * * *`) sends a **daily Telegram digest, silent on 0-visitor days**. No new site env var. **Verified live e2e:** deployed → all 4 sites 200 → real `POST /api/pv` landed a row → `pv-digest.py today` delivered a Telegram (`ok=True`) → `pv-digest.py` (yesterday) correctly silent → test row deleted (table back to 0). Chosen over the hosted Plausible dashboard because Ishank picked "build it all, no signup" (2026-07-16). Canonical script committed at `apps/web/scripts/pv-digest.py`.
