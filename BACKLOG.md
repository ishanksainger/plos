...
6g. [PR #29 · 2026-07-13 · claude — staged `active:false`, awaiting review/merge + go-live gate] **Daily Routine & Habit Tracker (`habit-tracker`, ₹399, Google Sheet, 16 tabs) — list on the site.** Handoff from Claude Desktop (`NIS_Habit_Previews/README-for-Claude-Code.md`). Sixth tracker, fourth link-delivered one — same recipe as 6c/6d/6f: `nis-data` + commerce-catalog entries + `/copy` `deliveryUrl` + a square cover at `public/trackers/habit-tracker.png`. **Staging `active:false` first** (invisible on the site — grid/home/sitemap/detail all gate on `getPurchasableTracker`), so the code merges with zero visible change; a single `active:true` flip takes it live once the three owner blockers clear: (1) master sheet `1f-HisJNG8jr…` set to "Anyone with link → Viewer"; (2) Welcome PDF's `{{COPY_LINK}}` placeholder replaced with the real link + re-uploaded (content lane); (3) ₹399 + thenispace.com confirmed final (✅ Ishank 2026-07-13). Price ₹399 + deep-forest accent `#274e37` (secondary amber `#F5A524`) confirmed by Ishank 2026-07-13. **Owner:** `claude` (wiring) + `human`/Claude Desktop (sheet sharing + Welcome PDF link).
