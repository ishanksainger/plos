...
6c. [in progress · 2026-06-17 · claude] **Indian Wedding Budget Planner (`wedding-budget`, ₹899, Google Sheet, 11 tabs)** — second live tracker. Content (name / copy / price / sheet) provided by Ishank from the `NIS_launch_kit`. **New delivery model:** force-copy Google Sheets link (the sheet carries a bound Apps Script, so it can't ship as a downloadable `.xlsx`) → new optional `deliveryUrl` field on the catalog `Tracker`; `/api/download` redirects there (token-gated, same as a file) whenever a product has no `storage_path`. Adds `nis-data` + commerce-catalog entries + a `commerce.products` row (storage_path null). **GO-LIVE GATE (only Ishank can do):** set the sheet's sharing to "Anyone with link → Viewer" + test the `/copy` link from a second Google account — until that's confirmed it stays unshipped (an unshared link = paid-but-broken delivery, the exact thing the deliverable-only rule forbids). Partially addresses the P0 "3 more trackers" item below.
