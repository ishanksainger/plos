...
- **Download-delivery hardening** `[in progress · 2026-06-05 · claude]` — buyer receipt emails link straight at `GET /api/download?token=` (counts a use + 302s to the file). Email link-scanners (Gmail/Outlook SafeLinks/Mimecast) prefetch links, so a buyer can lose downloads off their 5-use cap before they ever click. Fix: point the emailed link at a read-only `/download?token=` landing page (scanners load the page, only a human click burns a use); add a HEAD no-op + guarded increment on the API route. **Owner:** `claude`
