...
| **Dashboard** | Live summary, overdue/due/upcoming lists, category breakdown, financial pressure by month, needs-attention table (**Edit** opens same modal), recently completed, dual rings, **activity sparkline** (`activitySeries` on `/users/dashboard` — last 14 days of completions + recurring check-ins from `Event`). |
...
| **Scheduler** | Cron records state transitions into `Event` (currently **every minute** for dev — tune for prod); on new DUE/OVERDUE transitions creates **in-app** `Notification` rows. |
...
| **UI** | Dark-first UI, charts (SVG / custom), timeline hero clock animations, stat cards, etc. (CSS-heavy work documented in repo history, not duplicated here.) |
